...
- **Feature Specifications**: Comprehensive authentication and authorization, CRUD operations for users and roles, technician management including map view, work order management with multi-tab detail views, proposal approval system with NTE/technician cost fields, parts request management with workflow, file upload system for various document types, real-time chat for work order communication, integrated client payment system (down/full/partial with method tracking), technician payment request and management, invoice management with automatic tax calculations, financial analysis for profit/loss tracking, teams management for organizing technicians with team leads, a comprehensive notification system, and a **Job Inspection system** for pre-job site assessments with before photos, scope of work, technician requirements, overview status (NT / Agreed on Site / Needs Proposal), and admin status tracking workflow.
